 ...
 * @returns { Performence.init() 👇👇👇 } new Performence()
 *
 *
 ...
 *
 * @exmaple 请参考performence.md
*/
var Performence = /*#__PURE__*/function () {
  function Performence() {
    _classCallCheck(this, Performence);
    ...
    this.performenceTime = {}; // performence埋点数据集合

    this.navigationTime = {}; // 当前浏览器统计埋点数据集合
  } // 自定义添加标记的时间, 方便我们计算程序的运行耗时


  _createClass(Performence, [{
    key: "_mark",
    ...
    key: "_performenceTime",
    value: function _performenceTime(_b) {
      ...
      this._addEventListen(this.performenceTime, v, p, c, _b);
    } // 浏览器导航计时
    ...
    value: function _timeCompute3(s, _o, p, c) {
      ...
      if (this.log === true) {
        console.info("%cperformence \u548C navigation \u7EDF\u8BA1\u57CB\u70B9\u6570\u636E\uFF1A", "color:".concat(c, ";"));
    ...
    value: function init() {
      ...
      if ('performance' in window) {
        try {
          // 查阅基准点有无刷新
          console.log("%c".concat(new Date(performance.timeOrigin).toLocaleString(), "\u2014\u2014\u2014\u2014performance\u7EDF\u8BA1\u7CBE\u5EA6\u57FA\u51C6\u70B9\uFF1A").concat(performance.timeOrigin), 'color:#FAAD14;font-weight:bold'); // performence.timing计算（废弃，仍能取值，将来不再维护）

          var P1 = new Promise(function (resolve) {
            _this5._performenceTime(resolve);
    ...
    value: function handleBaseData() {
      ...
      for (var v in this.navigationTime) {
        ...
        if (!base[v] && this.performenceTime[v] && this.performenceTime[v].time) {
          base[v] = this.performenceTime[v].time;
        }
      }
...
window.__pm__ = new Performence();

var _default = new Performence();
